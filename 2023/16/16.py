...
def main():
    ...
    print('\npart 1:')
    print(part_1(data))
    #print(part_1(data, watch=True, delay=0))
    
    print('\nexample 2:')
    print(part_2(ex_data, watch=True, delay=0), '= 51?')
...
def part_2(data, watch: bool = False, delay: float = 0.05) -> int:
    ...
    n_cols = len(data[0])
    return max(
        light_it_up(data, start=Beam(p, v), watch=watch, delay=delay)
...
def light_it_up(data, start: Beam, watch: bool = False, delay: float = 0.05) -> int:
    beams = deque([start])
    lit = [[Direction(0) for p in row] for row in data]
    if watch:
        display_init(len(data))
    while beams:
        # Refresh display after we've cycled all the beams.
        if watch:
            display_back_up(data)
            display(data, lit, beams)
            sleep(delay)
...
def display_back_up(data) -> None:
    print(f'\x1b[{len(data)}F', end='')

def display_init(lines: int):
    term = os.get_terminal_size()
    if lines >= term.lines:
        print(f'Warning: grid len {lines} too tall for terminal {term.columns=} {term.lines=}')
    print(control.HIDE_CURSOR)
    print('\n' * lines, end='')

def display(data, lit, beams) -> None:
